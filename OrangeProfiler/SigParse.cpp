...
bool SigParser::ParseRetType()
{
    ...
    {
        NotifyVoid((sig_elem_type)ELEMENT_TYPE_VOID);
        pbCur++;
        goto Success;
    }
...
bool SigParser::ParseType()
{
    ...
    */
    ...
    case  ELEMENT_TYPE_CLASS: 
        ...
        if (!ParseTypeDefOrRefEncoded(&indexType, &index, & mdType))
            return false;
        
        NotifyTypeDefOrRef(indexType, index, mdType);      
        break;
    ...
    case  ELEMENT_TYPE_GENERICINST:
        ...
        NotifyTypeGenericInst(elem_type, indexType, index, number, mdType);
...
bool SigParser::ParseTypeDefOrRefEncoded(sig_index_type *pIndexTypeOut, sig_index *pIndexOut, mdToken * pUncompressedTok)
{
    // parse an encoded typedef or typeref

    sig_count encoded  = 0;

	if (NULL != pUncompressedTok)
		CorSigUncompressToken((PCCOR_SIGNATURE) pbCur, pUncompressedTok); 

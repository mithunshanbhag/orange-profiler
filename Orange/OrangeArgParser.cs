...
namespace OrangeClient
{
    ...
    public class OrangeArgParser
    {
        ...
        public OrangeArgParser(string arguments)
        {
            Init(arguments);
        }
        ...
        public OrangeArgParser(string arguments, string optionSpec)
        {
            Init(arguments);
        ...
        public ArgToken GetArgument(int index)
        {
            if(!Exists(index))
            {
               throw new OrangeShellException("Missing argument "+index);
            }
        ...
        private void Init(string arguments)
        {
            ...
            if(isInString)
            {
                throw new OrangeShellException("Arguments contain an unterminated string");
            }
        ...
        private void ParseOptions(string optionSpec)
        {
            ...
            for(i=0; i<m_args.Count;i++)
            {
                ...
                if ((m_args[i] as String).StartsWith("-") || (m_args[i] as String).StartsWith("/"))
                {
                    ...
                    if(availOptions.Contains(optName))
                    {
                        if((bool)availOptions[optName])
                        {
                            ...
                            if(i==m_args.Count-1)
                            {
                                throw new OrangeShellException(String.Format(CultureInfo.InvariantCulture, "option {0} needs argument",new Object[]{optName}));
                            }
                        ...
                        throw new OrangeShellException(string.Format("Invalid option '{0}' specified.",m_args[i]));
                 }
                else
                {
                    break;
    ...
    public class CommandArgument
    {
        ...
        public string Lookup(string input)
        {
            ...
            if(al.Count==0)
            {
                throw new OrangeShellException("Invalid command '"+input+"'.");
            }
            else if(al.Count==1)
            {
                return (string)al[0];
